    ...
    <body>
        ...
        <jsp:include page="../jspSegments/navbar.jsp"></jsp:include>
            <div class="container">
                ...
                <form action="/product?action=update&productId=${product.getProductId()}" method="post" class="create-product-form" id="create-product-form" enctype="multipart/form-data">
                    ...
                    <div class="is-form-group form-group mb-3">
                        ...
                        <select class="form-select is-form-control form-control shadow-none" id="account" name="account">
                            <c:forEach items="${requestScope.employees}" var="e">
                                <option <c:if test="${e.getAccount().equals(product.getAccountDTO().getAccount())}">selected</c:if> value="${e.getAccount()}">${e.getAccount()} - ${e.getLastName()} ${e.getFirstName()}</option>
                            </c:forEach>
                        </select>
                    </div>
                </c:if>
                ...
                <c:import url="../jspSegments/input.jsp">
                    ...
                    <%--<c:param name="value" value="${product.getProductImage()}"/>--%>
                </c:import>
                <c:import url="../jspSegments/input.jsp">
                ...
                <div class="is-form-group form-group mb-3">
                    ...
                    <select class="form-select is-form-control form-control shadow-none" id="typeId" name="typeId">
                        <c:forEach items="${requestScope.categories}" var="c">
                            <option ${c.getTypeId() == product.getCategory().getTypeId() ? "selected" : ""}  value="${c.getTypeId()}">${c.getCategoryName()}</option>
                        </c:forEach>
                    </select>
                </div>
                <c:import url="../jspSegments/input.jsp">
                ...
                <div class="text-center">
                    <button type="submit" form="create-product-form" class="btn btn-success">
                        <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor" class="bi bi-indent" viewBox="0 0 16 16">
